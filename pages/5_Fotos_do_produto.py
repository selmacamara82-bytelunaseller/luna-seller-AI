...
def botao_regenerar(chave, rotulo, prompt):
    if st.session_state.get(chave) and st.button(f"🔄 Gerar outra {rotulo}", key=f"regen_{chave}", use_container_width=True):
        try:
            contador = int(st.session_state.get(f"variacao_{chave}", 0)) + 1
            st.session_state[f"variacao_{chave}"] = contador
            variacoes = [
                "Faça uma versão VISUALMENTE MUITO DIFERENTE da anterior: novo enquadramento, nova posição do produto, nova distribuição dos acessórios e fundo diferente, mantendo o produto fiel.",
                "Crie uma nova composição com ângulo de câmera mais baixo ou três-quartos, iluminação diferente e cenário claramente novo. Não repita a composição anterior.",
                "Crie uma alternativa de catálogo com outro ponto de vista, mais espaço ao redor do produto e organização diferente dos acessórios. A imagem deve parecer de outra sessão fotográfica.",
            ]
            instrucao_variacao = variacoes[(contador - 1) % len(variacoes)]
            prompt_variado = prompt + " " + instrucao_variacao + " IMPORTANTE: a foto de referência pode mostrar o MESMO produto em mais de uma posição para demonstrar aberto e fechado; isso NÃO significa que existam duas unidades. Mostre somente UMA unidade do corpo principal. Cada acessório real pode aparecer no máximo uma vez."
            with st.spinner("Criando uma nova opção..."):
                gerar_imagem(chave, prompt_variado, "✅ Nova opção criada. Confira o resultado.")
            st.rerun()
        except Exception as erro:
...
prompt_principal = "Crie uma imagem quadrada profissional de catálogo usando SOMENTE o produto real da referência. Fundo branco puro. Produto principal grande no lado esquerdo ou centro-esquerda. No lado direito, até três círculos verticais mostrando detalhes REAIS. A referência pode mostrar o mesmo produto aberto e fechado; trate isso como UMA única unidade, não duas. Preserve rigorosamente formato, cor, peças e acessórios. Não invente nem duplique itens. Sem textos. Alta nitidez e aparência premium."
prompt_detalhes = "Crie uma imagem quadrada premium usando SOMENTE o produto real da referência. CENÁRIO OBRIGATORIAMENTE DIFERENTE DA FOTO PRINCIPAL: bancada clara de cozinha moderna ou mesa de madeira clara, iluminação natural de janela, ambiente claro e acolhedor. Mostre UMA única unidade do corpo principal e 2 a 4 closes de detalhes reais; os closes não contam como unidades extras. A referência pode mostrar o produto aberto e fechado, mas isso representa o mesmo item. Evite fundo preto, cinza escuro ou estúdio dramático. Preserve exatamente formato, cor e acessórios. Não invente nem duplique componentes. Sem texto. Alta nitidez e fotografia comercial elegante."
prompt_beneficios = "Crie uma imagem quadrada profissional usando SOMENTE o produto real. CENÁRIO OBRIGATORIAMENTE DIFERENTE DAS OUTRAS FOTOS: ambiente de home office ou mesa de café clara, com tons bege e creme, luz natural da manhã, elementos discretos como caderno, caneca ou planta ao fundo. Mostre APENAS UMA unidade do corpo principal do produto, montada ou aberta, mas nunca as duas formas ao mesmo tempo. A referência pode mostrar o mesmo produto em mais de uma posição; não interprete isso como duas unidades. Cada acessório real pode aparecer no máximo uma vez. Não use bancada preta, fundo escuro, fumaça ou aparência de estúdio. REGRA ABSOLUTA: não escreva benefícios, materiais, desempenho ou especificações que não tenham sido explicitamente confirmados pela vendedora. Na dúvida, faça a composição SEM TEXTO. Preserve exatamente o produto e não invente itens."
...
prompt_informativa = "Crie a quinta imagem profissional de marketplace usando SOMENTE o produto real da referência. CENÁRIO OBRIGATORIAMENTE DIFERENTE: fundo muito claro, minimalista, em branco quente ou bege suave, com organização visual de catálogo e bastante espaço negativo. Não use cenário de cozinha, ambiente externo ou fundo escuro. Mostre UMA única unidade do corpo principal e seus acessórios reais uma única vez cada. A referência pode mostrar o mesmo produto aberto e fechado; não trate essas duas visualizações como duas unidades. Faça uma composição limpa e informativa. NÃO escreva capacidade, medidas, material, desempenho, certificação, marca, modelo ou qualquer especificação não confirmada. Se não houver dados textuais seguros, faça a imagem SEM TEXTO, usando apenas linhas, setas discretas ou enquadramentos para destacar peças e detalhes existentes sem alegações."
