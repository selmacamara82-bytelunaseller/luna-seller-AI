...
# Acesso direto à nova área, sem interferir no envio ao Mercado Livre.
if st.button("📷 Ir para Fotos do produto", use_container_width=True):
    st.switch_page("pages/5_Fotos_do_produto.py")
...
titulo_aprovado = str(aprovada.get("titulo") or st.session_state.get("revisao_aprovada_titulo", "") or "").strip()
descricao_aprovada = str(aprovada.get("descricao") or st.session_state.get("revisao_aprovada_descricao", "") or "").strip()
...
st.text_area("Descrição pronta para conferência", value=descricao_aprovada, height=420, disabled=True, key=f"preparar_descricao_{anuncio_id}")
...
plano = {"id": anuncio_id, "titulo": titulo_aprovado, "descricao": descricao_aprovada, "palavras": palavras_aprovadas, "status": "pronto_para_conferencia_manual" if conteudo_pronto else "bloqueado_conteudo_incompleto", "enviar_ao_mercado_livre": False}
st.info("Confira o plano antes do envio. As palavras-chave ficam no Luna Seller para apoio e não são enviadas como um campo separado ao Mercado Livre.")
st.download_button("Baixar plano de atualização", data=json.dumps(plano, ensure_ascii=False, indent=2), file_name=f"plano_atualizacao_{anuncio_id or 'anuncio'}.json", mime="application/json", use_container_width=True)
...
if st.button("Confirmar e atualizar no Mercado Livre", type="primary", use_container_width=True, disabled=not (ha_alteracao and confirmacao_valida)):
    try:
        resultados = []
        if alterar_titulo:
